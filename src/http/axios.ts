...
function createInstance() {
  ...
  instance.interceptors.response.use(
    (response) => {
      ...
      // 如果返回的数据中有 error 字段，说明是错误响应
      if (apiData.error) {
        const errorData = apiData.error
      ...
      // 增强结构校验  或
      if (typeof apiData.code === "undefined" || typeof apiData.success === "undefined") {
        ElMessage.error("接口返回格式异常")
        return Promise.reject(new Error("Invalid response structure"))
      }

      // 如果没有 code, 代表这不是项目后端开发的 api ，返回抛出异常
      if (code === undefined) {
